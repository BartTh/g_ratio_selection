...
raw_data_dir = os.path.join(r'D:\Switchdrive\gratio\new_raw_data\train\images')  # Input data directory
...
for file in glob.glob(fr'{raw_data_dir}\*.jpg'):
    ...
    if not Path.exists(Path(raw_data_dir).joinpath(f'{Path(file).name[:-4]}.nii.gz')):
        ...
        nib.save(nib_img, os.path.join(raw_data_dir, f'{Path(file).name[:-4]}.nii.gz'))
